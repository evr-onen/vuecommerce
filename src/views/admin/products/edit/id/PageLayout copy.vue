            ...
            <div class="flex flex-col justify-end gap-x-4 h-full border-formPlaceholder border p-4">
              <div class="!w-full flex h-full flex-col justify-between">
                <button
                  ...
                  @click="() => values.gallery.length < 5 && pushGallery()"
                >
...
// ** prepare object and using setValues when page open and
watch(editProductData, () => {
  ...
  productImages.value = []
  for (let i = 0; i < 6; i++) {
    let tmpData = editProductData.value.product_images[i]
    if (tmpData?.priority == 0) {
      productMainImage.value = tmpData
    } else {
      let isInArray = productImages.value.findIndex((img) => img.id === tmpData?.id) !== -1
      if (tmpData && !isInArray) {
        productImages.value.push(tmpData)
      }
    }
  }
  ...
  for (let i = 0; i < productImages.value.length; i++) {
    if (productImages.value[i]) galleryPush({ fillerImage: productImages.value[i].path })
    imageRecordObj.value.push({ ...productImages.value[i], order: i + 1 })
  }
})

// ** swap galleri item and changed order in objitem
const swapGallery = (ind, swapInd) => {
  gallerySwap(ind, swapInd)
  if (imageRecordObj.value.find((imgData) => imgData.order === ind + 1)) {
    imageRecordObj.value.find((imgData) => imgData.order === ind + 1).order = swapInd + 1
    imageRecordObj.value.find((imgData) => imgData.order === swapInd + 1).order = ind + 1
  }
}

// ** push galleri item and add order in objitem
const pushGallery = () => {
  galleryPush({ file: null })
  imageRecordObj.value.push({ order: galleryFields.value.length })
}
// ** remove galleri item and changed order in after objitem
const removeGallery = (index, fieldKey) => {
  imageRecordObj.value.splice(fieldKey, 1)
  for (let i = index; i < imageRecordObj.value.length; i++) {
    imageRecordObj.value[i].order--
  }
  galleryRemove(index)
}
...
const onSubmit = handleSubmit((data) => {
  ...
  updateProduct({ ...data, id: editProductData.value.id, imageOrders: imageRecordObj.value })
