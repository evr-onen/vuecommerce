<template>
  <div
    class="wrapper group/card w-[215px] h-[375px] p-4 font-Krub border border-info rounded-lg shadow-sm hover:shadow-lg relative bottom-0 hover:bottom-2 hover:right-1 right-0 duration-300"
  >
    <div class="relative">
      <router-link
...
const emits = defineEmits(['toggleViewHandler', 'toggleVariantViewHandler', 'maxPrice'])
...
onMounted(() => {
  ...
  emits('maxPrice', cardData.value.currentPrice)
})
...
const addToCartHandler = () => {
  if (product.value.to_variant_products.length === 0) {
    ...
    emits('toggleVariantViewHandler', product.value.id)
  }
}
