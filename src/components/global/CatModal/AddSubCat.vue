<template>
  <div class="h-full">
    <h2 class="text-center mt-8">Create Sub Category</h2>
    <div class="w-full h-full">
      <form class="p-4 w-full h-full" @submit.prevent="onSubmit">
        <div class="w-full flex flex-col items-center relative">
          <div class="!w-full flex flex-col items-center">
            <label for="dropdown">Select a Main Category:</label>
            <Dropdown
              name="mainCat"
              v-bind="mainCat"
              :options="mainOptions"
        ...
        <div class="">
          <p class="text-center mt-4">Sub Category</p>
          ...
          <div class="flex flex-col items-center relative mt-8">
            ...
            <p class="text-xs text-red-500 absolute -bottom-4 left-8">{{ errors.catNameEN }}</p>
          </div>
        </div>
...
const catState = useCategoryStore()
...
const schema = yup.object().shape({
  ...
  mainCat: yup.number().required('please select a main category')
})
...
const mainCat = defineComponentBinds('mainCat')
...
const { getCats, createCats } = categories()
// getCats()
...
const onSubmit = handleSubmit((data) => {
  createCats({ name: { en: data.catNameEN, tr: data.catNameTR }, parent_id: data.mainCat })
  resetForm()
})
</script>
